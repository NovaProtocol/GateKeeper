...
Any other rule can be switched off with `rules.active`. An inactive rule is **skipped**, so the request falls through to the next matching rule in its group — normally the catch-all — rather than being refused. The catch-all itself cannot be switched off (`400 the catch-all cannot be deactivated`), because that would leave its group with no fallback and every path it does not name refused.
...
| `POST /manage/codes/{cid}/active`, `POST /manage/codes/{cid}/delete` | manage | Activate/deactivate a code with the row switch, or delete it permanently with the separate trash control against a typed `confirm_code` |
| `POST /manage/groups/{gid}/edit`, `POST /manage/rules/{rid}/edit` | manage | Edit a rule group (`name`, `domain`) or a rule (`path`, `action`); the same rule route carries the row's `active` switch |
...
| `PUT /api/groups/{gid}`, `PUT /api/rules/{rid}` | internal (`X-Internal-Api-Key`) | Update a group or rule; each field is validated only when present in the body (`rules.active` accepts booleans and `true`/`false`/`1`/`0`) |
