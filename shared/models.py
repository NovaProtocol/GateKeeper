...
class ApiKey(Base):
    ...
    whitelist: Mapped[Optional[str]] = mapped_column(Text, nullable=True)
    blacklist: Mapped[Optional[str]] = mapped_column(Text, nullable=True)
    expires_at: Mapped[Optional[dt.datetime]] = mapped_column(DateTime, nullable=True)
    created_at: Mapped[dt.datetime] = mapped_column(DateTime, server_default=func.now())
    last_used: Mapped[Optional[dt.datetime]] = mapped_column(DateTime, nullable=True)
